...
def get_data(url):
    ...
    if r.status_code == 500:
        logging.error("500 Code, trying up to 3 times", file=sys.stderr)
...
def get_replays(fc_profile):
    ...
    for i in range(0, int(config['replay_pages'])):
        ...
        logging.info(f"Getting page {page} results", file=sys.stderr)
        try:
            r = get_data(url)
            for replay in r.json()['data']:
                replays.append(replay)
        except Exception as e:
            logging.error("Failed after 3 attempts or unknown error, continuing", file=sys.stderr)
