...
class TestGcloud:
    ...
    @patch('fcreplay.gcloud.Path')
    @patch('fcreplay.gcloud.socket')
    def test_destroy(self, mock_socket, mock_path, mock_logging, mock_database, mock_requests):
        gcloud = self.setUp()
        ...
        gcloud.destroy_fcreplay()
        assert mock_requests.post.assert_called, "Should call post to destroy instance"
